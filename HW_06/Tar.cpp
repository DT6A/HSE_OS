...
void Tar::saveToTarget(string &target, string &rootDir)
{
  ...
  if (write(fileDescr, reinterpret_cast<const char *>(&size), sizeof(size)) != sizeof(size)) throw runtime_error("Failed to write to target file");
  for (string &name : names)
  {
    FileContainer fc(name, rootDir);
    if (ino2file.find(fc.stats.st_ino) == ino2file.end())
    {
      fc.saveToFile(fileDescr);
      ino2file[fc.stats.st_ino] = name;
    }
    else
...
{
  if (mkdir(target.c_str(),  S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH) != 0) throw runtime_error("Failed to create directory " + to_string(errno));
  ...
  for (size_t i = 0; i < size; ++i)
    containers[i].loadFromFile(fileDescr);
